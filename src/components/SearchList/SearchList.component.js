...
import { SET_BROWSING_TRUE } from "../../constants";
    ...
    return (
      <section className="search-list">
        <div className="search-list__info">
          <p>
            Results for...{" "}
            <span className="search-list__info__search-term">
              "{searchTerm}"
            </span>
          </p>
          <Button type="primary" size="large" onClick={() => setBrowsing()}>
            Back to browsing
          </Button>
        </div>
        <ListContainer list={searchList.results || []} />
        <ListControls
          prevUrl={searchList.previous}
          nextUrl={searchList.next}
          listType="searching"
        />
      </section>
    );
